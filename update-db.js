...
const prisma = new PrismaClient({ datasources: { db: { url: 'postgresql://postgres:FTMHTlcMAeiHbKoBqljSGyBDTbpjASVN@nozomi.proxy.rlwy.net:41033/railway' } } });
...
async function main() {
  ...
  for (const p of products) {
    ...
    const stock = isRupture ? 0 : (Math.floor(Math.random() * 40) + 15);
    await prisma.product.update({ where: { id: p.id }, data: { stock, image_url: IMAGES[p.reference] || null } });
